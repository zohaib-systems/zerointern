...
export function mapProject(row: Record<string, unknown>): Project {
  return {
    ...
    difficultyLevel: (row.difficulty_level as Project["difficultyLevel"]) ?? "beginner",
    ...
    skillsLearned: Array.isArray(row.skills_learned) ? row.skills_learned.map(String) : [],
    realWorldValue: row.real_world_value ? String(row.real_world_value) : null,
